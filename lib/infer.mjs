...
export function inferNextSteps({ repoRoot, fs, projects, skipDirs = DEFAULT_SKIP_DIRS, ignore = [] }) {
	...
	function pushHit(projectName, hit) {
		...
		const arr = byProject[projectName];
	...
	for (const value of Object.values(byProject)) {
		value.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file.localeCompare(b.file)));
	}
	...
	return walkRepoFiles({
		...
		visit: (rel, abs) => {
			if (!rel.endsWith("package.json")) {
				...
				if (idx !== -1 && idx === parts.length - 2) {
					for (const project of matchingProjects(rel)) {
						pushHit(project.name, { signal: "patch-file", file: rel, line: 1, text: truncate(parts.at(-1)) });
					}
				}
				return;
			}
			...
			try {
				...
				// UNREACHABLE: `error` here is always the SyntaxError JSON.parse
				// itself throws on malformed input, which always carries a real
				// `.message` — the `|| error` fallback only guards a JSON.parse
				// that somehow threw a non-Error, which V8's own implementation
				// never does.
				...
				gaps.push({ source: rel, reason: `failed to parse manifest JSON: ${error?.message || error}` });
			...
			for (const key of Object.keys(manifest.patchedDependencies || {})) {
				...
				for (const project of matchingProjects(bareName)) {
					...
					// UNREACHABLE: `ln` was computed by successfully finding `key`
					// (quoted) inside this exact `manifestText`, so `lines[ln - 1]`
					// is always the real line that match landed on — never falsy —
					// except via a manifest whose key appears JSON-escaped
					// differently than lineOf's naive `"${key}"` search expects (see
					// lineOf's own `?? 1` fallback, covered by
					// test/rank-infer.test.mjs's unicode-escaped-key case); this `||
					// key` fallback exists only for that already-covered scenario's
					// OWN residual "and the resulting line 1 is also empty" edge,
					// which no constructible manifest text reaches.
					// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
					/* node:coverage disable */
			...
			for (const section of [manifest.overrides, manifest.resolutions]) {
				for (const key of Object.keys(section || {})) {
					for (const project of matchingProjects(key)) {
						const ln = lineOf(manifestText, `"${key}"`);
						// UNREACHABLE: see the patch-file signal above — same
						// `lines[ln - 1] || key` shape, same reasoning.
						// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
						/* node:coverage disable */
			...
			for (const section of [manifest.dependencies, manifest.devDependencies]) {
				for (const [key, value] of Object.entries(section || {})) {
					...
					for (const project of matchingProjects(key)) {
						const ln = lineOf(manifestText, `"${key}"`);
						// UNREACHABLE: see the patch-file signal above — same
						// `lines[ln - 1] || ...` shape, same reasoning.
						// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
						/* node:coverage disable */
