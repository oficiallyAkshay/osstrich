...
const ATTEMPT_DIR_RE = /^(?<date>\d{4}-\d{2}-\d{2})-(?<slug>.+)-attempt-(?<attempt>\d+)$/;
...
function listAttempts(runsDir, slug, fs) {
	...
	for (const entry of entries) {
		...
		if (!m || m.groups.slug !== slug) {continue;}
		out.push({ name: entry.name, date: m.groups.date, attempt: Number(m.groups.attempt) });
	}
	return out;
}
...
export function openRun({ runsDir, slug, now = Date.now, fs = nodeFs, resume = false }) {
	...
	if (resume && attempts.length > 0) {
		attempts.sort((a, b) => (a.date === b.date ? b.attempt - a.attempt : (a.date < b.date ? 1 : -1)));
...
export async function finishRun(dir, record, { fs = nodeFs, now = Date.now, onRunEnd = null } = {}) {
	...
	if (onRunEnd) {
		try {
			...
			fs.appendFileSync(recordPath, `${JSON.stringify({ run_id: runId, gap: `hook_error: onRunEnd failed: ${error?.message || error}` })}\n`);
		}
	}
