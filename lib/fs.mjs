...
function isTransientGhFailure(error) {
	...
	return !hasCapturedExit || !error?.stdout || String(error.stdout).trim() === "";
}
...
export function isSkippedDir(rel, entryName, skipConfig) {
	return skipConfig.names.has(entryName) ? true : skipConfig.prefixes.some((p) => rel === p || rel.startsWith(`${p}/`));
}
