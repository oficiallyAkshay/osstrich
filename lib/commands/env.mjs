...
function runList(deps) {
  ...
  for (const name of vars) {
    // eslint-disable-next-line unicorn/no-computed-property-existence-check -- reports whether the var's VALUE is truthy, not merely whether the key exists.
...
function upsertEnvLine(existingText, name, value) {
  ...
  const isExists = Object.hasOwn(parsed, name);
  ...
  const nextLines = rawLines.map((line) => {
    if (isReplaced || !isExists) {return line;}
    const match = line.match(/^\s*(?<key>[A-Za-z_][A-Za-z0-9_]*)\s*=/);
    if (match && match.groups.key === name) {
      isReplaced = true;
      return `${name}=${value}`;
    }
    return line;
