...
export async function run(argv, deps) {
  ...
  for (const detection of detections) {
    ...
    if (dotenvxEncrypted && detection.name === 'dotenvx') {
  ...
  const isDotenvxPresent = Boolean(dotenvxTool?.found);

  if (!env.OSSTRICH_MODEL) {
    stdout.write(`OSSTRICH_MODEL unset: ${setHint({ dotenvxPresent: isDotenvxPresent, name: 'OSSTRICH_MODEL' })}\n`);
  }
  // eslint-disable-next-line unicorn/no-computed-property-existence-check -- checking the env var's VALUE is falsy (unset or empty), not merely that the key exists.
  if (providerVar && !env[providerVar]) {
    stdout.write(`${providerVar} unset: ${setHint({ dotenvxPresent: isDotenvxPresent, name: providerVar })}\n`);
  }
