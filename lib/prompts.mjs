...
export function slugFor(target) {
  ...
  let m = /^(?<owner>[\w.-]+)\/(?<repo>[\w.-]+)#(?<number>\d+)$/.exec(text);
  // UNREACHABLE: unlike the object-target branch above (whose .name/.repo
  // carry no guaranteed alnum character), this regex's `number` group is
  // `\d+` — at least one digit always survives slugPiece's collapse, so the
  // `|| 'target'` fallback below can never fire for a match from THIS
  // pattern (verified against 100-char symbols-only owner/repo strings).
  ...
  if (m) {return slugPiece(`${m.groups.owner}-${m.groups.repo}-${m.groups.number}`).slice(0, 64) || 'target';}
  ...
  m = /github\.com\/(?<owner>[\w.-]+)\/(?<repo>[\w.-]+)\/(?:issues|pull)\/(?<number>\d+)/.exec(text);
  // UNREACHABLE for the same reason as the disabled block above — this
  // pattern's `number` group is also `\d+`, guaranteeing a surviving alnum
  // character, so the consequent's `|| 'target'` fallback can never fire.
  // eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
  /* node:coverage disable */
  return m
    ? slugPiece(`${m.groups.owner}-${m.groups.repo}-${m.groups.number}`).slice(0, 64) || 'target'
    : slugPiece(text).slice(0, 64) || 'target';
  // eslint-disable-next-line unicorn/single-line-block-comment-style -- see the disable comment above; this exact form is required for Node to resume coverage tracking.
  /* node:coverage enable */
}
