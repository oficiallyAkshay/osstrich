...
const CLAIM_URL_RE =
	/https:\/\/github\.com\/(?<owner>[^/\s]+)\/(?<repo>[^/\s]+)\/(?<kind>pull|issues|releases\/tag)\/(?<id>[^\s)\]]+)/;

/** Maps `CLAIM_URL_RE`'s `kind` capture (the URL path segment) to this
 * module's own claim-kind vocabulary. `releases/tag` has no entry — it
 * falls through to "release", the only kind that isn't a literal path
 * segment name. */
const URL_SEGMENT_TO_KIND = { pull: "pr", issues: "issue" };
...
const HEADING_REPO_RE = /^#\s+(?<owner>[\w.-]+)\/(?<repo>[\w.-]+)\s*$/;
...
const GENERIC_GH_URL_RE = /github\.com\/(?<owner>[^/\s]+)\/(?<repo>[^/\s]+)\//;
 ...
 * number. */
const WORD_HASH_RE = /\b(?<word>pr|pull request|pull|issue)\b\s*#(?<number>\d+)\b/i;
 ...
 * REFERENCES` below). */
const OWNER_REPO_HASH_RE = /\b(?<repo>[\w.-]+\/[\w.-]+)#(?<number>\d+)\b/;
...
function resolveHeadingRepo(lines) {
	for (const line of lines) {
		...
		if (m) {return `${m.groups.owner}/${m.groups.repo}`;}
	}
	return null;
}
...
function resolveBareReference(lineText, repo) {
	...
	if (wordMatch) {
		if (!repo) {return null;}
		const word = wordMatch.groups.word.toLowerCase();
		const kind = word === "issue" ? "issue" : "pr";
		const path = kind === "pr" ? "pull" : "issues";
		return { url: `https://github.com/${repo}/${path}/${wordMatch.groups.number}`, kind, ambiguous: false };
	}
	const ownerRepoMatch = lineText.match(OWNER_REPO_HASH_RE);
	return ownerRepoMatch
		? { url: `https://github.com/${ownerRepoMatch.groups.repo}/pull/${ownerRepoMatch.groups.number}`, kind: "pr", ambiguous: true }
		: null;
}
 ...
 */
// Only ever `.test()`ed, never `.match()`ed for its groups — every group is
// non-capturing on purpose.
const CONDITION_RE =
...
export function extractClaims({ files, fs = nodeFs }) {
	...
	for (const file of files || []) {
		...
		for (const [index, lineText] of lines.entries()) {
			...
			const isCondition = CONDITION_RE.test(lineText);

			let url = urlMatch ? urlMatch[0] : null;
			let kind = urlMatch ? (URL_SEGMENT_TO_KIND[urlMatch.groups.kind] || "release") : "none";
			let isAmbiguous = false;

			if (!urlMatch) {
				const resolved = resolveBareReference(lineText, headingRepo || fallbackRepo);
				if (resolved) {
					({ url, kind, ambiguous: isAmbiguous } = resolved);
				}
			}
			...
			if (genericGh) {fallbackRepo = `${genericGh.groups.owner}/${genericGh.groups.repo}`;}

			if (!url && !isCondition) {continue;}
			const claim = { file, line: index + 1, text: lineText.trim(), url, kind, condition: isCondition };
			if (isAmbiguous) {claim.ambiguous = true;}
			claims.push(claim);
		}
	}
	return claims;
...
function parseClaimUrl(url) {
	// UNREACHABLE: `url || ""`'s fallback side. `recheckOneClaim` is this
	// function's only caller, and `recheckClaims` diverts every falsy-url
	// claim into `unconfirmable` ("no checkable artifact") before that call
	// is ever made — verified directly against url values 0, "", null, false
	// and NaN, every one of which stops at that filter.
	...
	if (!m) {return null;}
	const { owner, repo, kind: pathSegment, id } = m.groups;
	const kind = URL_SEGMENT_TO_KIND[pathSegment] || "release";
	return { owner, repo, kind, id };
}

function endpointFor({ owner, repo, kind, id }) {
	if (kind === "pr") {return `repos/${owner}/${repo}/pulls/${id}`;}
	return kind === "issue" ? `repos/${owner}/${repo}/issues/${id}` : `repos/${owner}/${repo}/releases/tags/${id}`;
}
...
async function releasePublishedAfter({ owner, repo, mergedAtIso, exec }) {
	try {
		...
		return releases.some((r) => !(r?.prerelease || r?.draft || !r?.published_at) && (new Date(r.published_at).getTime() > mergedAtMs));
	} catch {
		return false;
	}
}
...
async function liveStateFor(kind, data, { exec, owner, repo } = {}) {
	if (kind === "pr") {
		const isMerged = Boolean(data?.merged_at);
		const published = isMerged && exec ? await releasePublishedAfter({ owner, repo, mergedAtIso: data.merged_at, exec }) : false;
		return { open: data?.state === "open", merged: isMerged, published, fired: isMerged };
	}
	if (kind === "issue") {
	...
	return { open: false, merged: false, published: isPublished, fired: isPublished };
}

function isContradictedBy(assertion, state) {
	return assertion === "merged" ? !state.merged : (assertion === "open") && !state.open;
}
...
async function recheckOneClaim({ claim, exec }) {
	...
	try {
		...
		if (claim.ambiguous && parsed.kind === "pr") {
			...
			try {
				...
				return { bucket: "unconfirmable", entry: { ...claim, reason: `gh api ${endpoint} failed: ${error.message}` } };
			}
		} else {
	...
	if (assertion && isContradictedBy(assertion, state)) {
		return { bucket: "contradicted", entry: { ...claim, live: state, asserted: assertion } };
	}
	return ({ bucket: claim.condition && state.fired ? "fired" : "current", entry: { ...claim, live: state } });
}
...
export async function recheckClaims({ claims, exec = defaultExecFileAsync, maxCalls = RECHECK_MAX_CALLS_DEFAULT }) {
	...
	try {
		...
		for (const claim of toCheck) {
			result.unconfirmable.push({ ...claim, reason: `could not check gh api rate limit: ${error.message}` });
		}
		return result;
	}
