...
async function ensurePhase({ dir, deps, phase, verb, resume, run, readExisting, write }) {
  if (resume) {
    ...
    if (existing) {
      // UNREACHABLE: both call sites below always pass `verb` (every
      // buildPhaseTable() entry hardcodes its own `verb`, and the verdict
      // block passes `verb: 'verdict'` explicitly) — the `?? phase`
      // fallback only guards a future caller that forgets to.
...
function readPrsOpened(fs, filePath) {
  try {
    ...
    const m = /^prs_opened:\s*(?<count>\d+)/.exec(firstLine);
    return m ? Number(m.groups.count) : 0;
  } catch {
    return 0;
  }
}
...
async function cmdDiscover(args, deps) {
  ...
  const bottomN = flags.bottom === undefined ? undefined : Number(flags.bottom);
  const isResume = Boolean(flags.resume);
  const isTable = Boolean(flags.table);

  deps.stdout.write(`osstrich: config ${deps.configPath || 'no config'}\n`);
  const { dir } = await deps.openRun({ runsDir: deps.runsDir, slug, fs: deps.fs, resume: isResume });

  const { results, error } = await runPhases(dir, buildPhaseTable(deps, { bottomN }), deps, 'shortlist', { resume: isResume });
  // UNREACHABLE from cmdDiscover: buildPhaseTable() always returns the full
  // 4-phase table in dependency order, so every phase's own prereqs are
  // already present in `results` by the time runPhases checks them (see
  // runPhases' own doc comment above). The disk-fallback {error} this
  // guards only ever fires for cmdPhase's single-phase table below, which
  // the "standalone phases: prerequisite errors" test exercises.
  // eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
  /* node:coverage disable */
  if (error) {
  ...
  try {
    ...
    markPhase(dir, { stage: 'verdict', state: 'failed', error: `verdict.json unreadable: ${error_.message}` }, { fs: deps.fs });
    throw new OsstrichError('AGENT', `osstrich discover: verdict.json unreadable: ${error_.message}`, {
  ...
  const isHeadless = Boolean(flags.headless) || !deps.isTTY;
  const picked = await pickCandidates({ rows: pickerRows, headless: isHeadless, prompts: deps.prompts, stdout: deps.stdout });
  const pickedRows = picked.map((p) => verdict.rows.find((row) => row.name === p.name)).filter(Boolean);

  let prsOpened = 0;
  for (const row of pickedRows) {
    const slug2 = slugFor(row);
    // Builds run sequentially by design (see USAGE).
...
async function cmdPhase(name, args, deps) {
  ...
  if (!dir) {
    ...
    const opened = await deps.openRun({ runsDir: deps.runsDir, slug, fs: deps.fs, resume: isResume });
    ({ dir } = opened);
  }
  ...
  const { error } = await runPhases(dir, [targetPhase], deps, targetKey, { resume: isResume });
  if (error) {
    deps.stdout.write(`${error}\n`);
    return 1;
  }
...
async function cmdScrub(args, deps) {
  ...
  const result = typeof flags.text === 'string' ? (await deps.scrubText({ text: flags.text, termsText, exec: deps.exec, fs: deps.fs, repoRoot: deps.repoRoot })) : (await deps.scrubPaths({ paths: positional, termsText, exec: deps.exec, fs: deps.fs, repoRoot: deps.repoRoot }));
...
async function cmdBuild(args, deps) {
  ...
  if (target) {
    const slug = slugFor(target);
    // UNREACHABLE: unlike cmdDiscover/cmdPhase's own `--slug` flag (a raw,
    // unprocessed user string), `slug` here is ALWAYS slugFor()'s own
    // output — lowercased, every non-[a-z0-9] run collapsed to a single
    // hyphen, leading/trailing hyphens trimmed, and 'target' substituted
    // for an empty result. That construction can never produce a string
    // SLUG_RE rejects (verified against emoji, whitespace-only, all-symbol,
    // 200-char, and empty/nullish inputs) — this check only guards against
    // slugFor() itself changing contract, never against any target string
    // reachable from this CLI today.
    // eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
    /* node:coverage disable */
    if (!SLUG_RE.test(slug)) {
      deps.stdout.write(USAGE);
