...
export async function runAgentStage({
  ...
  try {
    ...
    // UNREACHABLE: this catch block's own completion (falling through to
    // `finally` after its `return`) has no code path a test can drive
    // independently of the return statement's own value, which is already
    // fully covered above — a Node coverage-instrumentation boundary at the
    // catch/finally seam, not a decision this function makes.
