...
function readCapped(fs, absPath, maxFileBytes) {
	...
	return text.length > maxFileBytes ? { skipped: true } : { text };
}
...
function loadLockCached({ fs, repoRoot, lockRel, lockCache, gaps }) {
	...
	try {
		...
		isCorrupt = true;
		gaps.push({ source: "lockfile", file: lockRel, error: error.message });
	}
	const result = { lock, corrupt: isCorrupt };
	lockCache.set(lockRel, result);
	return result;
}
...
function readNpmDependencies({ repoRoot, fs, gaps, allFiles, maxFileBytes }) {
	...
	for (const manifestRel of manifestRelPaths) {
		...
		try {
			...
			gaps.push({ source: "npm-manifest", file: manifestRel, error: `could not parse manifest JSON: ${error.message}` });
			continue;
		}
...
const RELEASE_URL_RE = /github\.com\/(?<owner>[\w.-]+)\/(?<repo>[\w.-]+)\/releases\/(?:download|tag)\/(?<rest>[^\s'"`)]+)/g;
const LITERAL_VERSION_RE = /^v?\d+[\w.-]*/;
const INTERPOLATED_CONST_RE = /\$\{(?<name>[A-Za-z_][A-Za-z0-9_]*)\}/;
...
function isTestPath(rel) {
	...
	return TEST_FILE_NAME_RE.test(parts.at(-1)) || parts.slice(0, -1).some((dir) => TEST_DIR_NAMES.has(dir));
}
...
function nearestConstValue(text, constName, nearLine) {
	const re = new RegExp(String.raw`\b${escapeRegExp(constName)}\s*=\s*["']([^"']+)["']`, "g");
	let best = null;
	let m = re.exec(text);
	while (m) {
...
function readBinaryPins({ repoRoot, fs, gaps, allFiles, maxFileBytes }) {
	...
	for (const fileRel of files) {
		...
		for (const m of text.matchAll(RELEASE_URL_RE)) {
			const { owner, repo: repoName, rest } = m.groups;
			const line = lineAt(text, m.index);
			const [verSeg] = rest.split("/", 1);
			...
			if (literal) {
				...
				if (interpolated) {
					version = nearestConstValue(text, interpolated.groups.name, line);
					if (version == null) {
						gaps.push({ source: "binary-pins", file: fileRel, error: `no "${interpolated.groups.name}" constant found in ${fileRel} for its interpolated release URL` });
					}
				} else {
...
const IMAGE_LINE_RE = /^\s*-?\s*image:\s*(?<ref>\S+)/;
...
const VENDORED_RE =
	/Vendored from github\.com\/(?<owner>[^/\s]+)\/(?<repo>[^\s@]+)\s*@\s*(?<ref>\S+)\s*\((?<date>[^)]+)\)/;
...
// `gaps` is deliberately not in this destructure: unlike its five sibling
// readers, this one never reports one (an unreadable/oversized file is
// "routine, not a gap" per the comments below) — but the caller still
// passes the same uniform `{ repoRoot, fs, gaps, allFiles, maxFileBytes }`
// shape to every reader, so the extra key on the call site is expected.
function readVendoredCopies({ repoRoot, fs, allFiles, maxFileBytes }) {
	const rows = [];
	for (const fileRel of allFiles.sort()) {
		...
		let match = null;
		for (const [i, line] of lines.entries()) {
			const m = line.match(VENDORED_RE);
			if (m) {
				matchedLine = i;
				match = m;
				break;
		...
		if (!match) {continue;}
		const { owner, repo: repoName, ref } = match.groups;
...
const PATCH_PACKAGE_FILE_RE = /^(?<package>.+)\+(?<version>[^+]+)\.patch$/;
...
async function readPatchPackageFiles({ allFiles, fetch, gaps, patchNumberByPkg }) {
	...
	const files = allFiles
		.filter((f) => {
			...
			return idx !== -1 && idx === parts.length - 2 && parts.at(-1).endsWith(".patch");
		})
		.sort();
	for (const fileRel of files) {
		...
		if (!m) {continue;}
		const rawPkg = base.slice(0, - 6); // strip ".patch"
...
const MD_HEADING_RE = /^(?<hashes>#{1,6})\s+(?<title>.+?)\s*$/;
const MD_HEADING_PATCH_WORD_RE = /\bpatch(?:es)?\b/i;
const MD_LIST_ITEM_RE = /^\s*(?:\d+\.|[-*])\s+(?<item>.+)$/;
...
function firstOwnerRepoToken(text) {
	// UNREACHABLE: this function's only caller, `resolveRepoFromText` below,
	// already guards on `text ?` — `text` is always truthy here, so the
	// `|| ""` fallback can never fire.
	// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
	/* node:coverage disable */
	const s = String(text || "");
	// eslint-disable-next-line unicorn/single-line-block-comment-style -- see the disable comment above; this exact form is required for Node to resume coverage tracking.
	/* node:coverage enable */
	const candidates = [];
	const ghMatch = /github\.com\/(?<owner>[\w.-]+)\/(?<repo>[\w-]+)/.exec(s);
	if (ghMatch) {candidates.push({ index: ghMatch.index, repo: `${ghMatch.groups.owner}/${ghMatch.groups.repo}` });}
	const bareRe = /(?<![\w./-])(?<owner>[\w-]+)\/(?<repo>[\w-]+)(?![\w./-])/;
	const bareMatch = bareRe.exec(s);
	if (bareMatch) {candidates.push({ index: bareMatch.index, repo: `${bareMatch.groups.owner}/${bareMatch.groups.repo}` });}
...
/**
Matches `name` whole-token and case-insensitively — no partial hit inside
a longer identifier on either side. The shared pattern behind every
"does this text mention that project's name" check below.
*/
function wholeTokenRegex(name) {
...
function firstProjectMentioned(text, candidateProjects) {
	...
	for (const p of candidateProjects) {
		// UNREACHABLE: the only `candidateProjects` array ever passed here is
		// `readMarkdownPatchLists`' own `priorProjects.filter((p) => p.name)`,
		// so every candidate already carries a truthy name.
		// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
		/* node:coverage disable */
		if (!p.name) {continue;}
		// eslint-disable-next-line unicorn/single-line-block-comment-style -- see the disable comment above; this exact form is required for Node to resume coverage tracking.
		/* node:coverage enable */
		const m = wholeTokenRegex(p.name).exec(text);
...
function resolveRepoFromText(text, candidateProjects) {
	return text ? firstOwnerRepoToken(text) ?? firstProjectMentioned(text, candidateProjects)?.repo ?? null : null;
}
...
function firstH1(lines) {
	for (const line of lines) {
		...
		if (m && m.groups.hashes === "#") {return m.groups.title;}
	}
	return null;
}
...
function nearestParagraph(preambleLines) {
	...
	if (current.length > 0) {paragraphs.push(current);}
	return paragraphs.length === 0 ? null : paragraphs.at(-1).join(" ");
}
...
function readMarkdownPatchLists({ repoRoot, fs, gaps, allFiles, maxFileBytes, priorProjects }) {
	...
	for (const fileRel of files) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (!heading || !MD_HEADING_PATCH_WORD_RE.test(heading.groups.title)) {continue;}
			...
			for (let j = i + 1; j < end; j++) {
				...
				if (item) {
					isStarted = true;
					current = { line: j + 1, parts: [item.groups.item] };
					items.push(current);
					continue;
				}
				if (line.trim() === "") {
					if (!isStarted) {preambleLines.push("");} // paragraph break, still in the preamble
					continue; // blank lines are tolerated, before or within the list
				}
				if (isStarted && /^\s+\S/.test(line)) {
					current?.parts.push(line.trim()); // an indented continuation of the current item
					continue;
				}
				if (isStarted) {break;} // an un-indented line ends the list — later content is not part of it
			...
			const headingText = heading.groups.title;
...
const USES_LINE_RE = /uses:\s*(?<repo>[^\s@'"]+\/[^\s@'"]+)@(?<tag>[^\s#]+)(?:\s*#\s*(?<comment>\S+))?/;
...
function readCiActions({ repoRoot, fs, gaps, allFiles, maxFileBytes }) {
	...
	for (const fileRel of files) {
		...
		for (const [i, line] of lines.entries()) {
			...
			if (!m) {continue;}
			const { repo, comment: version } = m.groups;
			if (!version) {
...
function readHostInstalls({ repoRoot, fs, gaps, hostsFile }) {
	...
	try {
		...
		gaps.push({ source: "host-installs", file: hostsFile, error: `hostsFile missing or unreadable: ${error.message}` });
		return rows;
	}

	const lines = text.split("\n");
	let isSawSeparator = false;
	for (const [i, line] of lines.entries()) {
		...
		if (!isSawSeparator) {continue;} // header row, before the separator
...
function parseGithubRepo(url) {
	...
	const m = url.match(/github\.com[:/](?<owner>[^/]+)\/(?<repo>[^/#]+?)(?:\.git)?(?:[/#].*)?$/);
	return m ? `${m.groups.owner}/${m.groups.repo}` : null;
}
...
async function attemptFetchJson(fetchImpl, url) {
	...
	try {
		...
		// UNREACHABLE (one of V8's two compiled copies of the `finally` below):
		// every path through the `try` above either returns or throws, and
		// both paths through this `catch` throw, so the try/catch can never
		// complete NORMALLY — only the abrupt-completion copy of `finally` is
		// ever reached, and the normal-completion copy stays permanently
		// unhit. Reproduced in isolation with every completion path exercised;
		// this is a structural artifact of the shape, not a missing test.
		...
		await Promise.all(
			batch.map(async (row) => {
				try {
					...
					gaps.push({ source: "github-metadata", file: row.repo, error: `gh api repos/${row.repo} failed: ${error_.message}` });
				}
				...
				try {
					...
					gaps.push({ source: "github-releases", file: row.repo, error: `gh api repos/${row.repo}/releases/latest failed: ${error_.message}` });
				}
			}),
		);
	}
...
export async function collectInventory({
	...
	try {
		...
		gaps.push({ source: "repo-walk", error: `failed to walk ${repoRoot}: ${error.message}` });
		allFiles = [];
	}
