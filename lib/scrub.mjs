...
function parseTermLine(rawLine) {
	...
	if (line.startsWith("re:")) {
		...
		return pattern ? { raw: line, kind: "regex", pattern } : null;
	}
...
function parseTerms(termsText) {
	return String(termsText || "")
		.split("\n")
		.map((rawLine) => parseTermLine(rawLine))
		.filter(Boolean);
}
...
function buildGitleaksConfig(termsText) {
	...
	for (const [index, term] of terms.entries()) {
		...
		lines.push(
			"[[rules]]",
			`id = "${id}"`,
			`description = "term: ${escapeTomlBasicString(term.raw)}"`,
			`regex = '''${regexBody}'''`,
			"",
		);
	}
...
function maskMatch(value) {
	// UNREACHABLE: `value ?? ""`'s fallback side. `toFinding` is this
	// function's only caller and already ends its own chain in `?? ""`
	// (`entry?.Match ?? entry?.Secret ?? ""`), so `value` is never nullish
	// here no matter what a gitleaks report entry does or doesn't carry.
...
function toFinding(entry, resolveFile) {
	return {
		...
		// UNREACHABLE: the ternary's else side. `runGitleaksOnSource` is the
		// only caller of `toFinding`, and both of ITS callers (`scrubText`,
		// `scrubPaths`) always hand it a `resolveFile` function — nothing in
		// this module reaches here with a non-function.
		// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
		/* node:coverage disable */
...
async function runGitleaksOnSource({ source, termsText, exec = defaultExecFileAsync, fs = nodeFs, tmpDir, repoRoot = process.cwd(), resolveFile }) {
	// UNREACHABLE: `tmpDir ||`'s fallback side. `scrubText` and `scrubPaths`
	// are this function's only callers and each resolves its own work dir
	// first, always passing it down as `tmpDir` — a real `mkdtempSync` never
	// returns a falsy path, so this function never makes a dir of its own.
	// eslint-disable-next-line unicorn/single-line-block-comment-style -- Node's test-runner coverage directive requires this EXACT single-line `/* node:coverage disable */` form; any reformatting (multiline, trailing text) stops Node from recognizing it at all.
	/* node:coverage disable */
	...
	try {
		...
		return { ok: false, error: `could not write scrub config: ${error.message}` };
	}
	...
	try {
		...
		return { ok: false, error: `gitleaks exec failed: ${error.message}` };
	}
	...
	try {
		...
		return { ok: false, error: `could not read gitleaks report: ${error.message}` };
	}
	...
	try {
		...
		return { ok: false, error: `could not parse gitleaks report: ${error.message}` };
	}
...
export async function scrubText({ text, termsText, exec = defaultExecFileAsync, fs = nodeFs, tmpDir, repoRoot = process.cwd() }) {
	...
	try {
		...
		return { ok: false, error: `could not write scrub input: ${error.message}` };
	}
