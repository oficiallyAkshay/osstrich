...
function readRawConfig(filePath, fs) {
  ...
  try {
    ...
    throw new OsstrichConfigError(`Malformed JSON in ${filePath}: ${error.message}`, {
      cause: error,
...
// Expand `${VAR}` / `${VAR:-default}` references against `env` FIRST, then
// expand a leading `~` (home directory) on the RESULT. Order matters: a
// default like `${OPENCLAW_STATE_DIR:-~/.openclaw}` only produces a leading
// `~` once the var is substituted (or falls back to its default) — tilde
// expansion must run after, on the substituted string, or that `~` is never
// recognized as leading anything and survives as a literal character.
export function expandString(value, { env, homedir }) {
  let out = value.replaceAll(
    /\$\{(?<name>[A-Za-z_][A-Za-z0-9_]*)(?::-(?<default>[^}]*))?\}/g,
    (_match, ...groups) => {
      const { name, default: def } = groups.at(-1);
...
export function loadConfig({ repoRoot, fs, env, homedir }) {
  ...
  const stateDir = resolveStateDir(env.OSSTRICH_STATE_DIR || (data.stateDir ?? DEFAULT_STATE_DIR), ctx);
