...
function extractKeywordsAndRefs(text, stopwordSet) {
	...
	const refs = new Set();
	const refRe = /#(?<number>\d+)/g;
	let m = refRe.exec(raw);
	while (m) {
		refs.add(m.groups.number);
		m = refRe.exec(raw);
	}
	const keywords = new Set();
	for (const word of raw.toLowerCase().split(/[^a-z0-9]+/)) {
		if ((word.length < MIN_KEYWORD_LEN) || stopwordSet.has(word) || /^\d+$/.test(word)) {continue;} // a bare number is never a useful title keyword
		keywords.add(word);
	}
...
async function fetchList({ kind, repo, exec, gaps }) {
	try {
		...
		try {
			...
			gaps.push(`gh ${kind} list returned unparseable JSON for ${repo} (soft): ${error.message}`);
			return [];
		}
		return raw.map((item) => normalizeItem(item)).sort((a, b) => a.number - b.number);
	} catch (error) {
		gaps.push(`gh ${kind} list failed for ${repo} (soft): ${error.message || error}`);
		return [];
	}
}
...
export async function shortlistUpstream({ projects, inferred, exec, concurrency = 4, minRemaining = 200, stopwords = [] }) {
	...
	for (let start = 0; !isStopped && start < targets.length; start += concurrency) {
		...
		await Promise.all(
			// `checked` is a single accumulator declared ONCE above the loop,
			// not re-scoped per iteration, and every batch is fully awaited
			// before the next one starts — there's no interleaving for this
			// closure to race.
	...
	return { byProject, budget: { checked, remainingAtStart, stopped: isStopped }, gaps };
}
