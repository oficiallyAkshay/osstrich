...
function buildAndClassifyRows({ allProjects, inventoryGaps, classification }) {
	...
	const starsValues = projects.map((p) => p.stars ?? -1);
	const starsRanks = competitionRank(starsValues);
	const downloadsValues = projects.map((p) => p.weeklyDownloads ?? null);
 ...
 * Returns `{ rows, bottom }`.
 */
function byCombinedThenName(a, b) {
	return a.combined === b.combined ? a.name.localeCompare(b.name) : a.combined - b.combined;
}
...
function orderAndSlice({ decorated, bottomN }) {
	...
	for (const row of decorated) {
		const isLeadsWithCommunity = row.override.length > 0 || row.class === "community" || row.class === "unclassified";
		(isLeadsWithCommunity ? communityFirst : companyLater).push(row);
	}
